// @ts-nocheck
/**
 * 合并行与列
 * @param data  需要合并的table
 * @param mergeCols  合并列配置
 * @param mergeRows  合并行配置
 */
export const buildSpans = (data, mergeCols, mergeRows) => {
  const spans = new Map<string, object>()
  const cols = initMergeCols(mergeCols)
  const rows = initMergeRows(mergeRows)
  for (const [i, row] of data.entries()) {
    buildMergeRowsSpan(data, i, row, rows, spans)
    buildMergeColsSpan(i, row, cols, spans)
  }
  return spans
}

/**
 * 初始化合并列配置
 * @param mergeCols  合并列配置
 */
const initMergeCols = (mergeCols) =>
  mergeCols?.map((col) => {
    const property = col.property || col
    return {
      rowspan: 1,
      rowIndex: 0,
      property,
      getValue: col.getValue || defaultGetValue(property),
    }
  })

/**
 * 初始化合并行配置
 * @param mergeRows 合并行配置
 */
const initMergeRows = (mergeRows) =>
  mergeRows?.map((row) => {
    const property = row.property || row
    return {
      colspan: 1,
      rowIndex: 0,
      property,
      getValue: row.getValue || defaultGetValue(property),
      // 关联的根行的索引值, 从0开始
      rootRowIndex: -1,
    }
  })

/**
 * 默认获取value的方法
 * @param property  列的prop属性值
 */
const defaultGetValue = (property) => {
  return (row) => row[property]
}

/**
 * 构建合并行数据
 * @param data      表格数据
 * @param rowIndex  当前行索引
 * @param row       当前行数据
 * @param mergeRows 合并行配置
 * @param spans     合并单元格数据
 */
const buildMergeRowsSpan = (data, rowIndex, row, mergeRows, spans) => {
  for (const column of mergeRows) {
    const rowValue = column.getValue(row)

    // 当前单元格合并数据
    const span = Object.assign({}, column)
    span.rowIndex = rowIndex
    span.rowspan = 1

    // 如果是第一行或者当前值与上一行当前列的值不同时，合并行数默认为1
    if (rowIndex === 0 || rowValue !== span.getValue(data[rowIndex - 1])) {
      spans.set(rowIndex + span.property, span)
      continue
    }

    // 如果当前值与上一行的值相同时，合并行数默认为0
    span.rowspan = 0
    // 获取上一行当前列的合并行数
    const preRowspan = spans.get(rowIndex - 1 + span.property)
    // 当前行关联的根行的索引值为：如果上一行是被合并的单元格，则取上一行当前列的关联的根行的索引值；否则取上一行索引值
    span.rootRowIndex =
      preRowspan.rowspan === 0 ? preRowspan.rootRowIndex : rowIndex - 1
    // 根据关联的根行的索引值更新对应的行合并数
    spans.get(span.rootRowIndex + span.property).rowspan += 1
    spans.set(rowIndex + span.property, span)
  }
}

/**
 * 构建合并列数据
 * @param rowIndex  当前行索引
 * @param row       当前行数据
 * @param mergeCols 合并列配置
 * @param spans     合并单元格数据
 */
const buildMergeColsSpan = (rowIndex, row, mergeCols, spans) => {
  for (let j = 0; j < mergeCols.length; j++) {
    // 单元格
    const column = mergeCols[j]
    // 当前单元格的数据
    const columnValue = column?.getValue(row)

    // 当前单元格合并数据
    const span = Object.assign({}, column)
    span.rowIndex = rowIndex
    span.colspan = 1

    const key = rowIndex + span.property
    setSpanOrUpdateColspan(spans, key, span)

    j = updateSpanAndGetNextColumnIndex(
      row,
      spans,
      rowIndex,
      mergeCols,
      j,
      columnValue
    )
  }
}

/**
 * 更新当前列的span数，并获取下一个待处理的列的索引
 * @param row             当前行数据
 * @param spans           合并单元格数据
 * @param rowIndex        当前行索引
 * @param mergeCols       合并列配置
 * @param rootColIndex    当前单元格索引
 * @param rootColValue    当前单元格数据
 */
const updateSpanAndGetNextColumnIndex = (
  row,
  spans,
  rowIndex,
  mergeCols,
  rootColIndex,
  rootColValue
) => {
  // 下一个待处理的列的索引
  let nextColumnIndex
  // 合并列长度
  const localColsLength = mergeCols.length
  for (let i = rootColIndex + 1; i < localColsLength; i++) {
    // 获取当前单元格
    const column = mergeCols[i]

    // 初始化合并数据
    const span = Object.assign({}, column)
    span.rowIndex = rowIndex
    span.colspan = 0

    // 如果指定单元格与当前单元格数据不匹配，则直接返回
    if (rootColValue !== column.getValue(row)) {
      // 更新指定单元格合并列数
      spans.get(rowIndex + mergeCols[rootColIndex].property).colspan =
        i - rootColIndex
      // 重置下一个待处理的列的索引为上一个列的索引，这样才能重新以当前单元格为开始进行匹配
      nextColumnIndex = i - 1
      break
    }

    // 如果直到最后一个单元格，其值也都一样
    if (i === localColsLength - 1) {
      // 更新指定单元格合并列数
      spans.get(rowIndex + mergeCols[rootColIndex].property).colspan =
        i - rootColIndex + 1
      // 重置下一个待处理的列的索引为合并列的最后一个索引值，这样才能结束下一个单元格匹配
      nextColumnIndex = localColsLength - 1
    }

    const key = rowIndex + span.property
    setSpanOrUpdateColspan(spans, key, span)
  }
  return nextColumnIndex
}

/**
 * 设置或更新span
 * @param spans
 * @param key
 * @param span
 */
const setSpanOrUpdateColspan = (spans, key, span) => {
  const sourceSpan = spans.get(key)
  if (sourceSpan) {
    sourceSpan.colspan = span.colspan
    return
  }
  spans.set(key, span)
}
